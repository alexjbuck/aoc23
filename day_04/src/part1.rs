...
fn process(input: &str) -> usize {
    input.lines().map(process_line).sum::<usize>()
}

fn process_line(line: &str) -> usize {
    let winners = line.split([':','|']).nth(1).unwrap().split_whitespace().map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>();
    let picks = line.split([':','|']).nth(2).unwrap().split_whitespace().map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>();
    let count: i32 = picks.iter().filter(|x| winners.contains(x)).count() as i32;
    2_f32.powi(count - 1).floor() as usize
}
